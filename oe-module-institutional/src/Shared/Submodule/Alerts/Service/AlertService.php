...
/**
 * src/Shared/Submodule/Alerts/Service/AlertService.php
 *
...
namespace OpenEMR\Modules\Institutional\Shared\Submodule\Alerts\Service;
...
final class AlertService
{
    ...
    public function computeAll(array $boardRows, array $latestVitals, int $facilityId): array
    {
        $alerts = array_merge(
            ...
            $this->computeObsFlaggedAlerts($facilityId)
        );
    ...
    public function computeObsBillingAlerts(int $facilityId): array
    {
        ...
        $billingService = new \OpenEMR\Modules\Institutional\ObservationStay\Submodule\ObsBilling\Service\ObsBillingService();
    ...
    // -----------------------------------------------------------------------
    // Extended observation alerts (device/RPM flagged readings)
    // -----------------------------------------------------------------------

    /**
     * Surfaces flagged oei_observation rows as alerts on the dashboard.
     *
     * Severity mapping:
     *   - value_numeric is null (text obs) or flag without bounds → WARNING
     *   - value > alert_high or < alert_low → CRITICAL if deviation >= 20% of bound,
     *     WARNING otherwise
     *
     * Only fires when the observations table exists (migration 0015 applied).
     * Limited to the last 24 hours to avoid alert fatigue from stale readings.
     *
     * @return array<int, array<string, mixed>>
     */
    private function computeObsFlaggedAlerts(int $facilityId): array
    {
        $obsRepo = new SharedObservationRepository();
        $flagged = $obsRepo->listFlagged($facilityId, hoursBack: 24, limit: 100);
        ...
        foreach ($flagged as $row) {
            ...
            $pid       = (int)($row['pid']        ?? 0);
            $typeName  = (string)($row['display_name']      ?? $row['obs_type_code'] ?? 'Observation');
            $value     = $row['value_numeric'];
            $unit      = (string)($row['unit']     ?? '');
            $lo        = $row['alert_low']  !== null ? (float)$row['alert_low']  : null;
            $hi        = $row['alert_high'] !== null ? (float)$row['alert_high'] : null;
            $observedAt = (string)($row['observed_datetime'] ?? '');
            $source    = (string)($row['source_type'] ?? 'MANUAL');

            // Compute severity — CRITICAL if >20% outside bound, else WARNING
            $severity = 'WARNING';
            if ($value !== null && ($lo !== null || $hi !== null)) {
                if ($hi !== null && $value > $hi && $hi > 0) {
                    if ((($value - $hi) / $hi) >= 0.20) {
                        $severity = 'CRITICAL';
                    }
                } elseif ($lo !== null && $value < $lo && $lo > 0) {
                    if ((($lo - $value) / $lo) >= 0.20) {
                        $severity = 'CRITICAL';
                    }
                }
            ...
            $valueStr = $value !== null
                ? rtrim(rtrim(number_format($value, 3, '.', ''), '0'), '.') . ($unit ? ' ' . $unit : '')
                : (string)($row['value_text'] ?? '—');

            $alertKey = 'OBS_FLAGGED:' . $episodeId . ':' . ($row['obs_type_code'] ?? '');
            ...
            $alerts[] = [
                ...
                'episode_id' => $episodeId,
                'pid'        => $pid,
                'message'    => $typeName . ': ' . $valueStr,
                'detail'     => xlt('Flagged reading') . ' · ' . $source
                              . ' · ' . substr($observedAt, 0, 16),
                'minutes'    => $observedAt
                              ? (int)round((time() - strtotime($observedAt)) / 60)
                              : null,
                'group'      => 'obs_flagged',
                'alert_key'  => $alertKey,
            ];
