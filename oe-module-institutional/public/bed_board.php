...
$epLocRepo = new EpisodeLocationRepository();
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if ($action === 'save_location') {
        ...
        if ($code !== '' && $name !== '') {
            $locRepo->upsert($facilityId, $code, $name, $type, $unit, $sort, $active, $notes);
        }
        header('Location: bed_board.php?facility_id=' . urlencode((string)$facilityId));
        exit;
    } elseif ($action === 'move_episode') {
        ...
        if ($episodeId > 0 && $pid > 0) {
            $epLocRepo->moveEpisode($episodeId, $pid, $eid, $facilityId, $locId, $locCode, $userId, $note);
        }
        header('Location: bed_board.php?facility_id=' . urlencode((string)$facilityId));
        exit;
    }
...
$locations = $locRepo->listActive($facilityId);
$episodes = $episodeRepo->fetchBoard($facilityId);
$currentLoc = $epLocRepo->listCurrentByFacility($facilityId);
$locByEpisode = [];
        ...
        <div class="list-group list-group-flush">
          <?php foreach ($locations as $loc): ?>
            <div class="list-group-item">
              ...
              <div class="small text-muted"><?= htmlspecialchars((string)$loc['code']) ?> • <?= htmlspecialchars((string)$loc['location_type']) ?><?= !empty($loc['unit_name']) ? (' • ' . htmlspecialchars((string)$loc['unit_name'])) : '' ?></div>
            </div>
          <?php endforeach; ?>
                  ...
                  <td>
                    <form method="post" class="d-flex gap-2">
                      ...
                      <select name="location_id" class="form-select form-select-sm">
                        ...
                        <?php foreach ($locations as $loc): ?>
                          <option value="<?= htmlspecialchars((string)$loc['id']) ?>"><?= htmlspecialchars((string)$loc['code']) ?> — <?= htmlspecialchars((string)$loc['name']) ?></option>
                        <?php endforeach; ?>
                      </select>
                      <input name="location_code" class="form-control form-control-sm" style="max-width: 90px;" placeholder="<?= xla("Code") ?>">
