...
/**
 * public/hbc/incident.php — HBC Incident Reports
 *
 * Thin wrapper over the shared IncidentController.
 * Home-based care incidents: falls, medication errors, unexpected
 * changes in condition, safeguarding concerns in the home setting.
 * IncidentRepository writes to oei_incident_report (episode-agnostic).
 */
...
<?php if ($data['flash']): ?>
<div class="alert alert-success py-2 mt-2"><?= htmlspecialchars($data['flash']) ?></div>
<?php endif; ?>

<div class="mb-3 mt-2">
  <button class="btn btn-danger btn-sm"
          data-bs-toggle="modal" data-bs-target="#newIncidentModal">
...
<div class="table-responsive">
  <table class="table table-sm table-hover">
    ...
    <tr class="<?= ($inc['mandatory_required'] && !$inc['mandatory_report_sent']) ? 'table-warning' : '' ?>">
      <td class="text-nowrap"><?= htmlspecialchars(date('M j H:i', strtotime($inc['incident_datetime']))) ?></td>
      ...
      <td>
        ...
        <?php if ($inc['mandatory_required']): ?>
          <span class="badge bg-danger ms-1"
                title="<?= xlt('Mandatory report required') ?>">!</span>
        <?php endif; ?>
      </td>
      <td>
        <span class="badge bg-<?= match($inc['severity']) {
            'CRITICAL' => 'danger', 'HIGH' => 'warning',
            'MODERATE' => 'info',   default => 'secondary' } ?>">
          <?= htmlspecialchars($inc['severity']) ?>
        </span>
      </td>
      <td>
        <?php if ($inc['mandatory_report_sent']): ?>
          <span class="text-success small">✔ <?= xlt('Sent') ?></span>
        <?php elseif ($inc['mandatory_required']): ?>
          <form method="POST"
                action="<?= htmlspecialchars($_hbcBase . 'incident.php?facility_id=' . $facilityId
            ...
            <button class="btn btn-warning btn-sm py-0"
                    style="font-size:.75rem;"><?= xlt('Mark Sent') ?></button>
          </form>
        <?php else: ?>
        ...
        <div class="modal-body row g-3">
          <div class="col-md-6">
            ...
            <input type="number" name="episode_id" class="form-control"
                   value="<?= $episodeId ?: '' ?>"
                   placeholder="<?= xla('Episode #') ?>" required>
          </div>
          <div class="col-md-6">
          ...
          <div class="col-12">
            <label class="form-label fw-semibold"><?= xlt('Location in Home / Address') ?></label>
            <input type="text" name="location_description" class="form-control"
                   placeholder="<?= xla('e.g. Living room, staircase, bathroom…') ?>">
          </div>
          <div class="col-12">
            <label class="form-label fw-semibold"><?= xlt('Narrative') ?></label>
            <textarea name="narrative" class="form-control" rows="3"
                      placeholder="<?= xla('Factual description of what occurred…') ?>"></textarea>
          </div>
          <div class="col-12">
            ...
            <textarea name="corrective_action" class="form-control" rows="2"
                      placeholder="<?= xla('Immediate actions taken…') ?>"></textarea>
          </div>
        </div>
        <div class="modal-footer">
          <button type="submit" class="btn btn-danger"><?= xlt('Submit Report') ?></button>
          <button type="button" class="btn btn-secondary"
                  data-bs-dismiss="modal"><?= xlt('Cancel') ?></button>
        </div>
      </form>
    </div>
  </div>
