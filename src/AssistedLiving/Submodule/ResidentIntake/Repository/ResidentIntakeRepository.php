...
/**
 * src/AssistedLiving/Submodule/ResidentIntake/Repository/ResidentIntakeRepository.php
 *
 ...
 *             tables reference: form_care_plan.encounter, forms.encounter,
 *             and any other encounter-anchored clinical artifacts.
 *             It is NOT form_encounter.id (the row PK).
 *
 ...
 *             class_code = 'AMB'  (ambulatory/residential encounter)
 *             pos_code   = 13     (Assisted Living Facility, CMS Place of Service)
 *
 ...
 *             in the OpenEMR chart timeline and encounter form list.
 *
 *   Step 4 — INSERT oei_episode (type = 'AL').
 *
 *   Step 5 — INSERT oei_al_episode, storing $encounterNum as encounter_id.
 */
final class ResidentIntakeRepository
{
    ...
    public function admitResident(
        ...
        if (!$formEncounterId) {
            error_log("[OEI] AL form_encounter INSERT failed — pid={$pid}");
            return 0;
        }
        ...
        // Step 5 — oei_al_episode (store encounter NUMBER, not row id)
        sqlInsert(
    ...
    /** Return the encounter NUMBER for an AL episode (form_encounter.encounter). */
